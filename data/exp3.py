...
class Hat:
    ...
    def draw(self, draw_times):
        if (
            len(self.contents) < draw_times
        ):  # Lines 15-30 are to make it where you can enter whatever amount of draw times and it'll still draw them.
            ...
            if appe > len(self.balls):
                ...
                for a in ap:
                    self.contents.append(a)
        v = random.sample(
            self.contents, draw_times
        )  # Line 31-38 is giving you the balls to be drawn at random and removing them from the hat, or if hat is empty replacing them into the hat.
        self.content_drew = v
...
def experiment(hat, expected_balls, num_balls_drawn, num_experiments):
    ...
    while (
        num_experiments > 0
    ):  # Lines 45-51 are breaking down the balls drawn from list format to the dictionary format.
